...
class RealTimeTrigger(QtWidgets.QWidget):
    ...
    def display_eve0diff(self):
        ...
        '''
        self.min_eve0diff, self.max_eve0diff = np.nanmin([np.nanmin([-10, np.nanmin(self.eve_ave_current['ESP_0_7_DIFFS']) - np.abs(np.nanmin(self.eve_ave_current['ESP_0_7_DIFFS']) * .5), -np.nanmax(self.eve_ave_current['ESP_0_7_DIFFS'])]), np.nanmin([-10, np.nanmin(self.eve_slow_current['slow_diffs']) - np.abs(np.nanmin(self.eve_slow_current['slow_diffs']) * .5), -np.nanmax(self.eve_slow_current['slow_diffs'])])]), np.nanmax([np.nanmax([10, np.nanmax(self.eve_ave_current['ESP_0_7_DIFFS']) * 2]), np.nanmax([10, np.nanmax(self.eve_slow_current['slow_diffs']) * 1.5])])
        self.line_min_eve0diff, self.line_max_eve0diff = np.nanmin([np.nanmin([-10, np.nanmin(self.eve_ave_current['ESP_0_7_DIFFS']) - np.abs(np.nanmin(self.eve_ave_current['ESP_0_7_DIFFS']) * .5), -np.nanmax(self.eve_ave_current['ESP_0_7_DIFFS'])]), np.nanmin([-10, np.nanmin(self.eve_slow_current['slow_diffs']) - np.abs(np.nanmin(self.eve_slow_current['slow_diffs']) * .5), -np.nanmax(self.eve_slow_current['slow_diffs'])])]), np.nanmax([np.nanmax([11, np.nanmax(self.eve_ave_current['ESP_0_7_DIFFS']) * 3]), np.nanmax([11, np.nanmax(self.eve_slow_current['slow_diffs']) * 2])])

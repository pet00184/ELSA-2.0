   ...
   ####################################################################################
    ...
    def _update(self):
        ...
        if self.new_data:
            ...
            # elif self._flare_prediction_state == "triggered": #testing out not doing the sound effect
                #self.trigger_sound_effect.play()
            elif self._flare_prediction_state == "launched":
    ...
    def temp_plot_update(self):
        if self.goes.shape[0]>70:
            self.time_tags = [pd.Timestamp(date).timestamp() for date in self.goes.iloc[-70:]['time_tag']]
            self.new_temp = np.array(self.goes.iloc[-70:]['Temp'])
        else: 
            self.time_tags = [pd.Timestamp(date).timestamp() for date in self.goes['time_tag']]
    ...
    def em_plot_update(self):
        if self.goes.shape[0]>70:
            self.time_tags = [pd.Timestamp(date).timestamp() for date in self.goes.iloc[-70:]['time_tag']]
            self.new_em = np.array(self.goes.iloc[-70:]['emission measure'])
        else: 
            self.time_tags = [pd.Timestamp(date).timestamp() for date in self.goes['time_tag']]
    ...
    def update_trigger_plot(self): 
        ...
        if self.flare_summary.shape[0]!=0:
            if self.flare_summary['Trigger'].iloc[-1] in list(self.goes['time_tag'].iloc[-70:]):
            ...
            if self.flare_summary['Trigger'].iloc[-1] not in list(self.goes['time_tag'].iloc[-70:]):
                self.flare_trigger_plot.setData([self.time_tags[0]]*2, [lower, higher])
                self.flare_trigger_plot.setAlpha(0, False)
            if pd.Timestamp(self.flare_summary['Realtime Trigger'].iloc[-1]).timestamp() > pd.Timestamp(self.goes['time_tag'].iloc[-70]).timestamp():
    ...
    def update_temp_em_launch_plots(self):
        ...
        if self.flare_summary.shape[0] != 0:
            ...
            if hasattr(self,"coming_launch_time") and (list(self.goes['time_tag'])[-70]<=self.coming_launch_time):
            ...
            if hasattr(self,"coming_launch_time") and (list(self.goes['time_tag'])[-70]>self.coming_launch_time):
                ...
                if hasattr(self,"_launched"):
                    del self._launched
            #if self.flare_summary['FOXSI Obs Start'].iloc[-1] not in list(self.goes['time_tag'].iloc[-70:]):
            if hasattr(self,"coming_launch_time") and (list(self.goes['time_tag'])[-70]>self.coming_launch_time_hic): 

...
class QTimeWidget(QWidget):
    ...
    app.exec()
    """
    def __init__(self, ANNA=False, parent=None, **kwargs):
        ...
        if ANNA:
            ...
            self._layout.addWidget(self._label_local, 1, 0)
            self._layout.addWidget(self._label_alaska, 2, 0)
        else:
            self._layout.addWidget(self._label_utc, 0, 0)
    ...
    def _get_times(self):
        ...
        self._alaska_str = (datetime.datetime.now(pytz.timezone('America/Anchorage'))).strftime(self._datetime_str_format)
